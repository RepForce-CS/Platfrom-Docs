...
```bash cURL
curl -X GET "https://app.repforce.ai/api/user/whatsapp/session-status?sender_id=12&recipient_phone=+1234567890" \
...
const response = await fetch(
  `https://app.repforce.ai/api/user/whatsapp/session-status?${params}`,
  {
    headers: {
...
response = requests.get(
    'https://app.repforce.ai/api/user/whatsapp/session-status',
